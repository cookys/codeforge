...
pub fn ingest(ctx: &db::Context, path: &Path) -> Result<usize> {
    ...
    for (i, section) in sections.iter().enumerate() {
        let text = if i == 0 { section.trim() } else { &format!("## {}", section.trim()) };
        if text.len() < 20 { continue; }

        let signal = Signal::new(text.to_string(), SignalSource::Ingest {
    ...
    if count == 0 && content.len() >= 20 {
        let signal = Signal::new(content.trim().to_string(), SignalSource::Ingest {
            path: path.display().to_string(),
        });
        writer.append(&signal)?;
        count = 1;
